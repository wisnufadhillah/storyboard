import React, { useEffect } from "react"; // 1. Tambah useEffect
...
export default function FeedbackScreen({ finalStory, onReset }) {
    // --- LOGIKA AUDIO (Baru) ---
    useEffect(() => {
        // Pastikan file "hebat.mp3" sudah ada di folder public/audio/
        const audio = new Audio("/audio/hebat.mp3");

        // Putar audio saat halaman muncul
        audio.play().catch((error) => {
            console.log("Gagal memutar audio:", error);
        });

        // Cleanup: Matikan suara kalau user keluar/reset
        return () => {
            audio.pause();
            audio.currentTime = 0;
        };
    }, []); // Array kosong [] artinya jalan 1 kali saja saat muncul
    ...
    return (
        <div className="h-full flex flex-col items-center justify-center p-8 text-center">
            ...
            {/* Review Cerita (Layout Box Kuning) */}
